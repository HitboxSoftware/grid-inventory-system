...
namespace Hitbox.Stash.UI
{
    ...
    public class InventoryUISlotGrid : InventoryUIAbstractGrid
    {
        ...
        {
            if (!base.AssignGrid(newGrid, generateUI)) return false;
        ...
        {
            Vector2Int[] positions = Grid.GetItemPositions(uiItem.InvItem);
            foreach (Vector2Int slot in positions)
            {
        ...
        protected override void ItemUpdated(InventoryItem invItem)
        {
            if (Grid == null)
            {
        ...
        {
            if (!slots.ContainsKey(gridPos)) return Vector2.zero;
            return slots[gridPos].transform.position;
        }
        ...
        public InventoryUISlot GetSlotFromPositions(Vector2Int position)
        {
